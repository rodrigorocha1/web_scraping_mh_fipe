...
class AvaliadorArvoreDecisao(Avaliador):
    ...
    def gerar_grafico_underfit_overfit(self, dados: dict):
        ...
        # Curvas
        ax.plot(param_range, train_rmse, marker='o', linewidth=2, label='RMSE Treino (Viés)')
        ax.plot(param_range, val_rmse, marker='o', linewidth=2, label='RMSE Validação (Generalização)')

        # Gap (variância)
        ax.fill_between(param_range, train_rmse, val_rmse, alpha=0.2, label='Gap Treino × Validação (Variância)')

        # Linha vertical no melhor depth
        ax.axvline(x=best_depth, linestyle='--', linewidth=2, label=f'Melhor max_depth = {best_depth}')

        # Marcar ponto ótimo
        ax.scatter(best_depth, best_rmse, s=120, zorder=5)

        # Anotações
        ax.text(best_depth + 0.5, best_rmse * 1.03,
                f'Mínimo RMSE (CV)\nRMSE ≈ {best_rmse:,.0f}', fontsize=10)
        ax.text(param_range[0], max(val_rmse) * 0.95, 'UNDERFITTING\n(alto viés)', fontsize=11, ha='left')
        ax.text(param_range[-1], min(train_rmse) * 1.05, 'OVERFITTING\n(alta variância)', fontsize=11, ha='right')

        # Labels finais
        ax.set_xlabel('max_depth (Complexidade do Modelo)')
        ax.set_ylabel('RMSE')
        ax.set_title('Árvore de decisão — Diagnóstico de Overfitting vs Underfitting')
        ax.legend()
        ax.grid(True)
        fig.tight_layout()
