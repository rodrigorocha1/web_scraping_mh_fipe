...
class AvaliadorRegressaoLinearRegularizada(Avaliador):
    ...
    def obter_dados_curva_validacao(
        ...
        if not hasattr(regressor, "alpha"):
            raise ValueError(
                "Este avaliador só suporta modelos regularizados (Ridge, Lasso, ElasticNet)"
            )
    ...
    def obter_resultados_modelo(
        ...
        coeficientes = dict(
            enumerate(map(float, regressor.coef_))
        )
    ...
    def gerar_grafico_underfit_overfit(self, dados: Dict[str, Any]) -> None:
        ...
        plt.semilogx(alpha, train_rmse, marker="o", label="Treino (Viés)")
        plt.semilogx(alpha, val_rmse, marker="o", label="Validação (Generalização)")
        ...
        plt.tight_layout()
        plt.savefig(
            f"fig/gerar_grafico_over_under/regularizada/"
    ...
    def obter_resultado_grid_search(self, grid_search: GridSearchCV) -> Dict[str, Any]:
        ...
        return {
            "best_params": {k: to_native(v) for k, v in best_params.items()},
